...
exports.createSparklineSvg = createSparklineSvg;
function createSparklineSvg(values, width = 1040, height = 260, options = {}) {
    if (values.length === 0) {
    ...
    const line = points
        ...
        .join(" ");
    const area = `${line} L${points[points.length - 1].x.toFixed(2)} ${baselineY.toFixed(2)} L${points[0].x.toFixed(2)} ${baselineY.toFixed(2)} Z`;
