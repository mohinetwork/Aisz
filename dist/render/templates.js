...
exports.buildSingleCardHtml = buildSingleCardHtml;
exports.buildTopCollageHtml = buildTopCollageHtml;
...
const THEME_STYLES = {
    violet_pulse: {
        ...
        bodyBackground: "radial-gradient(1100px 620px at 12% -16%, rgba(183, 121, 255, 0.2), transparent 58%), radial-gradient(900px 500px at 88% 12%, rgba(125, 152, 255, 0.1), transparent 65%), #03020a",
        frameBackground: "linear-gradient(158deg, rgba(18, 12, 36, 0.99) 0%, rgba(7, 5, 18, 0.99) 70%, rgba(3, 2, 8, 1) 100%)",
    ...
    aurora_stripe: {
        ...
        bodyBackground: "radial-gradient(900px 520px at 88% -8%, rgba(255,117,157,0.18), transparent 58%), radial-gradient(700px 420px at 12% 18%, rgba(187,129,255,0.1), transparent 62%), #05040a",
        frameBackground: "linear-gradient(155deg, rgba(15,10,24,0.985) 0%, rgba(7,5,11,0.99) 72%, rgba(3,2,5,1) 100%)",
    ...
    dark_black_white: {
        ...
        bodyBackground: "radial-gradient(1000px 560px at 35% -20%, rgba(255,255,255,0.08), transparent 60%), radial-gradient(700px 420px at 90% 5%, rgba(255,255,255,0.05), transparent 70%), #020202",
        frameBackground: "linear-gradient(152deg, rgba(16,16,16,0.99) 0%, rgba(7,7,7,0.99) 72%, rgba(2,2,2,1) 100%)",
    ...
    neon_glass: {
        id: "neon_glass",
        fontFamily: "'Space Grotesk', 'Manrope', 'Segoe UI', sans-serif",
        bodyBackground: "radial-gradient(960px 520px at 8% -14%, rgba(46,255,174,0.19), transparent 58%), radial-gradient(760px 460px at 92% 8%, rgba(84,215,255,0.08), transparent 64%), #020405",
        frameBackground: "linear-gradient(150deg, rgba(9,17,16,0.985) 0%, rgba(2,4,5,0.99) 70%, rgba(1,2,2,1) 100%)",
...
function getStyle(themeId) {
    return THEME_STYLES[themeId];
}
function percentColor(value, style) {
    return value >= 0 ? style.positive : style.negative;
}
function getTileBackground(themeId, index) {
...
function buildThemeRows(themeId, coin, style) {
    ...
    return `
      <div class="stats">
        <div class="stat-tile">
          ...
          <div class="stat-value">${(0, format_1.formatUsd)(sevenDayHigh)}</div>
        </div>
        <div class="stat-tile">
          <div class="stat-label">7D LOW</div>
          <div class="stat-value">${(0, format_1.formatUsd)(sevenDayLow)}</div>
        </div>
        <div class="stat-tile">
          <div class="stat-label">UPDATED</div>
          <div class="stat-value">${(0, format_1.formatUtcTimestamp)(coin.fetchedAtIso)}</div>
        </div>
      </div>
`;
}
function buildOrbitHudSingleCardHtml(payload) {
    const { coin } = payload;
    ...
    const sparkline = (0, sparkline_1.createSparklineSvg)(coin.chart7d, 760, 220, {
      ...
      <div class="frame">
        <div class="main-grid">
          <div class="left-panel">
            <div class="left-header">
              <div>
                <div class="coin-label">${(0, format_1.escapeHtml)(coin.symbol)}</div>
                <div class="coin-sub">${(0, format_1.escapeHtml)(coin.name)} / Tether</div>
              </div>
              <div class="theme-mark">VIOLET PULSE</div>
            </div>

            <div class="price">${(0, format_1.formatUsd)(coin.priceUsd)}</div>

            <div class="change-row">
              <div class="change-badge">${(0, format_1.formatPercent)(coin.change24h)}</div>
              <div class="change-amount">${(0, format_1.formatUsd)(Math.abs((coin.priceUsd * coin.change24h) / 100))}</div>
            </div>
        ...
        <div class="bottom-stats">
          ...
          <div class="stat">
            ...
            <div class="stat-value">${(0, format_1.formatPercent)(coin.change7d)}</div>
          </div>
        </div>
      </div>
    </div>
...
function renderTopTile(coin, index, themeId, style) {
    const pctColor = percentColor(coin.change24h, style);
    return `
<div class="tile" style="background:${getTileBackground(themeId, index)}">
  <div class="tile-top">
    <div class="coin-pill">${(0, format_1.escapeHtml)(coin.name)}</div>
    <div class="symbol">${(0, format_1.escapeHtml)(coin.symbol)}</div>
  </div>
  <div class="price">${(0, format_1.formatUsd)(coin.priceUsd)}</div>
  <div class="change" style="color:${pctColor}">${(0, format_1.formatPercent)(coin.change24h)}</div>
</div>`;
}
function buildSingleCardHtml(payload) {
    ...
    const sparkline = (0, sparkline_1.createSparklineSvg)(coin.chart7d, 1570, 400, {
        lineColor: sparklineLineColor,
        fillTopColor: sparklineFillTop,
        fillBottomColor: sparklineFillBottom
    });
    const pairLabel = `${(0, format_1.escapeHtml)(coin.name)} / USDT`;
    ...
    <div class="canvas">
      <div class="frame">
        <div class="content">
          <div class="header">
            <div class="coin-meta">
              <div class="coin-icon">${(0, format_1.escapeHtml)(symbolGlyph(coin.symbol))}</div>
              <div>
                <div class="coin-code">${(0, format_1.escapeHtml)(coin.symbol)}</div>
                <div class="coin-sub">${pairLabel}</div>
              </div>
            </div>
            <div class="theme-pill">${(0, format_1.escapeHtml)(BOT_BRAND_LABEL)}</div>
          </div>
          ...
          <div class="metrics">
            ...
            <div class="badge" style="color:${dailyColor}">${(0, format_1.formatPercent)(coin.change24h)}</div>
...
function buildTopCollageHtml(payload) {
...
<html>
  ...
  <body>
    <div class="canvas">
      <div class="frame">
        <div class="title">Top 9 Crypto Snapshot • ${(0, format_1.escapeHtml)((0, themes_1.getThemeLabel)(payload.themeId))}</div>
        <div class="subtitle">Updated ${(0, format_1.formatUtcTimestamp)(payload.fetchedAtIso)} UTC</div>
